...
public class Loan {
	...
	public double GetPMT() {
		...
		double n = getLoanPaymentCnt();
		double p = getLoanAmount();
		double f = getLoanBalanceEnd();
		boolean t = isbCompoundingOption();
	...
	public double getTotalPayments() {
		...
		for(int i=0; i<loanPayments.size(); i++) {
			tot = tot + loanPayments.get(i).getPayment() + loanPayments.get(i).getAdditionalPayment();
		}
		return tot;
	}
	...
	public double getTotalInterest() {
		...
		for(int i=0; i<loanPayments.size(); i++) {
			interestTotal = interestTotal + loanPayments.get(i).getInterestPayment();
		}
