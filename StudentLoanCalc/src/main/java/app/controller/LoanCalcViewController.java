...
public class LoanCalcViewController implements Initializable {
	...
	private boolean ValidateData() {
		...
		double loanNum = Double.parseDouble(LoanAmount.getText());
		if(Math.signum(loanNum) != 1.0) {
			contentText.append("Loan Amount negatvie or improper data type\n");
			
					
		}
		
		if (InterestRate.getText().trim().isBlank()) {
			contentText.append("Interest Rate Required\n");
			
		
		}
		double interestNum = Double.parseDouble(InterestRate.getText());
		if(Math.signum(interestNum) != 1.0 || interestNum<1 || interestNum>30) {
			contentText.append("Interest is not within bounds or improper data type\n");
			
				
		}
		
		int yearN = Integer.parseInt(NbrOfYears.getText());
		if(NbrOfYears.getText().trim().isBlank() || yearN <= 0) {
			contentText.append("NbrOfYears is not within bounds or improper data type\n");
			
		
		}
	
		double addPay = Double.parseDouble(AdditionalPayment.getText());
		if(addPay<0) {
			contentText.append("AdditionalPayment is not within bounds or improper data type\n");
		...
		if(escrow<0) {
			contentText.append("EscrowAmount is not within bounds or improper data type\n");
